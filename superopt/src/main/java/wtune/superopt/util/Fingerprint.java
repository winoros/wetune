...
public class Fingerprint {
  ...
  private static class OpFingerprintBuilder {
    ...
    private Fingerprint build(Op op, int limit) {
      ...
      final boolean dedup =
          op.kind() == OpKind.PROJ && ((Proj) op).isDeduplicated()
              || op.kind() == OpKind.SET_OP && ((Union) op).isDeduplicated();
  ...
  private static class PlanFingerprintBuilder {
    ...
    private char identifierOf(int node) {
      ...
      else if (kind == PlanKind.SetOp) return getOpIdentifier(OpKind.SET_OP, isDedup(plan, node));
